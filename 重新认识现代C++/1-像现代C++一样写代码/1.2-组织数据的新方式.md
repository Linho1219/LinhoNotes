...
这会把「目前输入了多少人」「最多允许多少人」和「数组实际有多大」分成三个容易混淆的概念。数量超过限制会失败，数量很少时又会留下大量空位。因此，接下来我们介绍如何使用标准库容器管理这些细节。
...
`std::array` 的成员函数这里不再逐一列举，可以查阅 [cppreference：`std::array`](https://cppreference.com/cpp/container/array)。也可以阅读 [菜鸟教程：C++ 容器类 `<array>`](https://www.runoob.com/cplusplus/cpp-libs-array.html)。
...
可以前往 [cppreference：`std::vector`](https://cppreference.com/cpp/container/vector) 了解更多成员函数和迭代器失效规则。也可以阅读 [菜鸟教程：C++ 容器类 `<vector>`](https://www.runoob.com/cplusplus/cpp-libs-vector.html)。
...
可以前往 [cppreference：`std::basic_string`](https://cppreference.com/cpp/string/basic_string) 了解更多成员函数。也可以阅读 [菜鸟教程：C++ 标准库 `<string>`](https://www.runoob.com/cplusplus/cpp-libs-string.html)。
...
可以把这两类视图直观地理解为「对一段连续数据的轻量描述」，实现中通常包含地址和长度等信息。它们要求数据连续，不拥有源数据，也不会延长源对象的生命周期；保存一个指向已经销毁对象的 `span` 或 `string_view` 会产生悬空视图。因此，它们适合作为函数参数或用于短期观察。
