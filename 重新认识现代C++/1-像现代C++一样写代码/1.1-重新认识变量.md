...
可以把**对象**（object）理解为一块**具有类型、生命周期的存储，上面存着一个值**。变量是通过名字指代对象的一种方式，但对象不一定有名字。例如已经有了 `int score = 60;` 之后，表达式 `score + 5` 也会产生一个临时结果，它的类型是 `int`、值是 `65`。
...
但请注意：我们说它「具有」类型、生命周期，并不意味着程序会在运行时的内存中记录类型、生命周期信息。这些额外信息**存在于编译阶段的编译器内部**。编译器在生成机器码时，已经知道这些对象的类型并「想好了」它们应该怎样操作。
...
机器指令中直接规定了应该怎样操作哪些比特。在运行时，没有人关心也没有人知道这段地址是怎样的类型、应该如何操作，因为所有的「剧本」都已经在机器码里写好了。
...
花括号初始化与原本 `=` 初始化的最大区别是：**花括号会拒绝一部分可能丢失信息的隐式转换**。标准明确规定，列表初始化中需要进行窄化转换时，程序不合法。
...
C++20 其，`auto` 还能用于定义函数参数类型，使其拥有泛型或者说函数模板的性质。我们会在后续章节中详细介绍，现在留个印象即可。例如：
...
这是为什么？因为这里的 `1` 默认为 `int`，左移 63 位超出了 `int` 的宽度，这是一个 UB，所以连这个 `0` 都不能保证。
...
`constexpr` 则进一步表示某个值或计算可以在编译期得到结果。`constexpr` 值的地位和字面量是一样的，可以出现在任何字面量可以出现的地方，例如开数组。
...
这里的 `static_var` 使用 `static` 声明，在首次经过其声明时初始化，属于静态存储期。其生命周期**不会自动结束**，而且在所有后续调用中，初始化将被跳过。静态局部变量会在程序正常结束时自动销毁。
...
返回的地址仍可能是一个数字，但它指向的对象已经结束生命周期。继续解引用就是悬空访问，即俗称的「野指针」。
