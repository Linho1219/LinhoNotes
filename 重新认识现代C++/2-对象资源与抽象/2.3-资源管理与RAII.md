...
以后增加的提前返回和异常也可能绕过释放。现代 C++ 的基本答案是：不要让一段控制流负责记忆清理，让对象的生命周期负责。

## RAII 将资源绑定到对象

RAII 是 Resource Acquisition Is Initialization 的缩写，通常译为「资源获取即初始化」。它的核心是：
...
RAII 的重点不是「必须放在栈上」，而是资源的所有权由具有确定生命周期的对象表达。一个 RAII 对象本身也可以被另一个对象拥有。

## 标准库已经在使用 RAII

我们此前使用的 `std::vector` 和 `std::string` 都是资源管理类型。它们内部可能申请动态内存，但调用者不需要手写 `delete`。
...
如果业务需要确认最终写入或关闭是否成功，仅依赖析构函数还不够，因为析构函数不适合报告可恢复错误。可以在正常路径显式执行 `flush` 或 `close` 并检查状态，析构函数仍作为所有退出路径上的安全网。
...
动态分配并不是创建对象的默认方式。下面的对象完全可以直接存在于当前作用域：
...
优先使用普通对象和标准库容器。只有对象生命周期需要独立于当前作用域、需要运行时多态，或数据结构本身需要间接关系时，才进一步考虑动态分配。

## `unique_ptr` 表达独占所有权

必须动态创建单个对象时，`std::unique_ptr<T>` 表示只有一个所有者：
...
// auto second = first; // 编译错误：不能复制所有权

auto second = std::move(first); // 转移所有权
```

转移后，`second` 拥有对象，`first` 变为空。`std::unique_ptr` 对移动后状态作出了明确保证，因此可以用 `if (first)` 检查。
...
如果两个对象互相持有 `shared_ptr`，引用计数可能永远无法归零：
...
struct Node {
    ...
    std::shared_ptr<Node> parent; // 可能与 children 构成环
};
```
