...
设有 $\boldsymbol f(\boldsymbol \xi_0, \boldsymbol \eta_0) = \boldsymbol 0 \in \mathbb R^q$。如有 $\bold D \boldsymbol f_{\boldsymbol \eta} (\boldsymbol \xi_0, \boldsymbol \eta_0)$==非奇异==，且 $\boldsymbol f(\boldsymbol x, \boldsymbol y) \in  C^1(\mathfrak D_x \times \mathfrak D_y, \mathbb R^q)$则：
...
接下来我们就需要计算
$$
\dot{\varphi}(\theta) = [\bold D \boldsymbol f(x + \theta \Delta \boldsymbol x) \Delta \boldsymbol x]^{\text{T}} [\boldsymbol f(\boldsymbol x + \Delta \boldsymbol x) - \boldsymbol f(\boldsymbol x)] .
$$
根据 Cauchy-Schwarz 不等式，两个向量的内积不超过它们的模长之积，有
$$
|\boldsymbol f(\boldsymbol x + \Delta \boldsymbol x) - \boldsymbol f(\boldsymbol x)|_{\mathbb R^n} ^ 2 \le |\bold D \boldsymbol f(\boldsymbol x + \theta \Delta \boldsymbol x) \Delta \boldsymbol x|_{\mathbb R^n} |\boldsymbol f(\boldsymbol x + \Delta \boldsymbol x) - \boldsymbol f(\boldsymbol x)|_{\mathbb R^n}.
$$
再根据矩阵范数的三角不等式，有
$$
|\boldsymbol f(\boldsymbol x + \Delta \boldsymbol x) - \boldsymbol f(\boldsymbol x)|_{\mathbb R^n} \le |\bold D \boldsymbol f(\boldsymbol x + \theta \Delta \boldsymbol x)|_{\mathbb R^{n \times n}} |\Delta \boldsymbol x|_{\mathbb R^n} \le \sup_{\theta \in [0, 1]} |\bold D \boldsymbol f(\boldsymbol x + \theta \Delta \boldsymbol x)|_{\mathbb R^{n \times n}} |\Delta \boldsymbol x|_{\mathbb R^n}.
$$
...
$$
\boldsymbol \phi_{\boldsymbol \xi}(\boldsymbol \eta): B_{\mu}(\boldsymbol \eta) \ni \boldsymbol \eta \mapsto \boldsymbol \phi_{\boldsymbol \xi}(\boldsymbol \eta) \stackrel {\triangle}{=} \boldsymbol \eta - (\bold D_{\boldsymbol \eta} \boldsymbol f)^{-1} (\boldsymbol \xi_0, \boldsymbol \eta_0) \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta) \in \mathbb R^q
$$
易证：
$$
...
根据有限增量公式，有估计
$$
\left|\boldsymbol \phi_{\boldsymbol \xi}(\boldsymbol \eta) - \boldsymbol \phi_{\boldsymbol \xi}(\boldsymbol \eta_0) \right| \le \sup_{\theta \in (0, 1)} \left| \bold D \boldsymbol \phi_{\boldsymbol \xi}(\boldsymbol \eta_0 + \theta(\boldsymbol \eta - \boldsymbol \eta_0)) \right|_{\mathbb R^{q \times q}} |\boldsymbol \eta - \boldsymbol \eta_0|_{\mathbb R^q}.
$$
考虑到
$$
\bold D \boldsymbol \phi_{\boldsymbol \xi}(\boldsymbol \eta) = \boldsymbol I_q - (\bold D_{\boldsymbol \eta} \boldsymbol f)^{-1}(\boldsymbol \xi_0, \boldsymbol \eta_0) \bold D_{\boldsymbol \eta}\boldsymbol f(\boldsymbol \xi, \boldsymbol \eta).
$$
估计
$$
\begin{align}
& \ \ \ \ |\bold D \boldsymbol \phi_{\boldsymbol \xi} (\boldsymbol \eta)|_{\mathbb R^{q \times q}} \\
& = \left| \boldsymbol I_q - (\bold D_{\boldsymbol \eta}\boldsymbol f)^{-1}(\boldsymbol \xi_0, \boldsymbol \eta_0) \bold D_{\boldsymbol \eta} \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta) \right|_{\mathbb R^{q \times q}} \\
& = \left| (\bold D_{\boldsymbol \eta} \boldsymbol f)^{-1}(\boldsymbol \xi_0, \boldsymbol \eta_0) [\bold D_{\boldsymbol \eta} \boldsymbol f (\boldsymbol \xi_0, \boldsymbol \eta_0) - \bold  D_{\boldsymbol \eta} \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta)] \right|_{\mathbb R^{q \times q}} \\
& \le \left| (\bold D_{\boldsymbol \eta} \boldsymbol f)^{-1}(\boldsymbol \xi_0, \boldsymbol \eta_0) \right|_{\mathbb R^{q \times q}} \left| \bold D_{\boldsymbol \eta} \boldsymbol f (\boldsymbol \xi_0, \boldsymbol \eta_0) - \bold D_{\boldsymbol \eta} \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta) \right|_{\mathbb R^{q \times q}}.
\end{align}
$$
...
$$
|\bold D \boldsymbol \phi_{\boldsymbol \xi}(\boldsymbol \eta)|_{\mathbb R^{q \times q}} < 1.
$$
...
$$
\left| \bold D_{\boldsymbol \eta} \boldsymbol f(\boldsymbol \xi_0, \boldsymbol \eta_0) - \bold D _{\boldsymbol \eta} \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta) \right|_{\mathbb R^{q \times q}} < |\bold D_{\boldsymbol \eta} \boldsymbol f(\boldsymbol \xi_0, \boldsymbol \eta_0)|_{\mathbb R^{q \times q}}.
$$
...
> [!note]
>
> 在此处粗体的 $\bold D$ 表示 Jacobi 阵，手写体的 $\mathfrak D$ 表示定义域。
...
& \le \sup_{\theta \in (0, 1)} \left| \bold D_{\boldsymbol \xi}(\boldsymbol \eta_1 + \theta(\boldsymbol \eta_2 - \boldsymbol \eta_1)) \right|_{\mathbb R^{q \times q}} |\boldsymbol \eta_2 - \boldsymbol \eta_1|_{\mathbb R^q} \\
...
& = \bold D_{\boldsymbol \xi} \boldsymbol \phi(\boldsymbol \xi, \boldsymbol \eta(\boldsymbol \xi)) \Delta \boldsymbol \xi + \bold D_{\boldsymbol \eta} \boldsymbol \phi (\boldsymbol \xi, \boldsymbol \eta(\boldsymbol \xi)) [\boldsymbol \eta(\boldsymbol \xi + \Delta \boldsymbol \xi) - \boldsymbol \eta(\boldsymbol \xi)] + o \left ( \sqrt{|\Delta \boldsymbol \xi|_{\mathbb R^m}^2 + |\boldsymbol \eta_{\boldsymbol \xi + \Delta \boldsymbol \xi} - \boldsymbol \eta_{\boldsymbol \xi}|_{\mathbb R^m}^2} \right ) \\
& = (\bold D_{\boldsymbol \eta} \boldsymbol f)^{-1}(\boldsymbol \xi_0, \boldsymbol \eta_0) · \bold D_{\boldsymbol \xi} \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta_{\boldsymbol \xi}) · \Delta \boldsymbol \xi + (\boldsymbol I_q - (\bold D_{\boldsymbol \eta} \boldsymbol f)^{-1} (\boldsymbol \xi_0, \boldsymbol \eta_0) · \bold D_{\boldsymbol \eta} \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta_{\boldsymbol \xi})) [\boldsymbol \eta(\boldsymbol \xi + \Delta \boldsymbol \xi) - \boldsymbol \eta(\boldsymbol \xi)] + o(|\Delta \boldsymbol \xi|_{\mathbb R^m}). \\
\end{align}
$$

则有
$$
(\bold D_{\boldsymbol \eta} \boldsymbol f)^{-1} (\boldsymbol \xi_0, \boldsymbol \eta_0) · \bold D_{\boldsymbol \eta} f (\boldsymbol \xi, \boldsymbol \eta_{\boldsymbol \xi}) · [\boldsymbol \eta_{\boldsymbol \xi + \Delta \boldsymbol \xi} - \boldsymbol \eta_{\boldsymbol \xi}] = (\bold D_{\boldsymbol \eta} f)^{-1} (\boldsymbol \xi_0, \boldsymbol \eta_0) · \bold D_{\boldsymbol \xi} \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta_{\boldsymbol \xi}) · \Delta \boldsymbol \xi (1 + o(1)).
$$
设定 $\bold D_{\boldsymbol \eta} \boldsymbol f$ 在 $(\boldsymbol \xi_0, \boldsymbol \eta_0)$ 的一个邻域内非奇异，则有
$$
\boldsymbol \eta(\boldsymbol \xi + \Delta \boldsymbol \xi) - \boldsymbol \eta(\boldsymbol \xi) = (\bold D_{\boldsymbol \eta} \boldsymbol f)^{-1} (\boldsymbol \xi, \boldsymbol \eta_{\boldsymbol \xi}) \bold D_{\boldsymbol \xi} \boldsymbol f(\boldsymbol \xi, \boldsymbol \eta_{\boldsymbol \xi}) \Delta \boldsymbol \xi + o(\Delta \boldsymbol \xi).
$$
这就是可微性。$\square$
