...
一长为 $l$，质量为 $m$ 的直杆，可绕通过其一端的水平光滑轴在竖直平面内作定轴转动，在杆的另一端固定着一质量为 $m$ 的小球，如图所示。现将杆由水平位置无初转速地释放。则杆刚被释放时的角加速度 $\alpha_0=$ $\underline{\hspace{3em}}$，杆与水平方向夹角为 $60^\circ$ 时的角加速度 $\alpha=$ $\underline{\hspace{3em}}$。
