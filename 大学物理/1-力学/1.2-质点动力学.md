...
作用力方向与 $\Delta v$ 方向相同。故作用力方向为与水平夹角 $53^\circ$ 向右上。
...
质点在几个力的作用下，沿着曲线 $\begin{cases}x=t\\y=t^2\end{cases}$ 运动，其中一个力为 $\vec F=5t\vec j$，则这个力在 $t=1\operatorname{s}$ 到 $t=2\operatorname{s}$ 时间内做的功为 $\underline{\hspace{3em}}$。
