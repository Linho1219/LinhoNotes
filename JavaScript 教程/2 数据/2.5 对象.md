...
> [!tip]
>
> 日常实践中，修改对象的引用并不常见。所以，除非有改动引用的需求，否则使用 `const` 来声明对象是良好的实践——因为 `const` 不会限制对属性和方法的追加、改动与删除，而且常常可以避免潜在的逻辑错误。
...
另外，需要注意的是，`delete` 命令只能删除对象本身的属性，无法删除继承的属性（[1.4 JavaScript 实现](../1%20%e5%85%a5%e9%97%a8/1.4%20%e5%ae%9e%e7%8e%b0#%e5%9f%ba%e4%ba%8e%e5%8e%9f%e5%9e%8b) 中已经提到了一些有关继承的内容，后面的章节会介绍）。
