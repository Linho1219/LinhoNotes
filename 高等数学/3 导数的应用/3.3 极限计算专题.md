...
> [!warning]
>
> 在利用极限的运算法则中的**乘法、乘方或除法**法则时，还另外需要注意：拆出部分极限不可为零，否则计算过程非法或不严谨。即使是在 有界函数乘无穷小 的情况下，也应该写在同一个极限符号内说明，不可通过拆出一个极限为零的部分来求取极限。
...
=&\lim_{x\to0}e^{\frac1{e^x-1}\cdot{\ln(1+x)-x\over x}} \\
=&\:e^{\lim\limits_{x\to0}\frac1{e^x-1}\cdot{\ln(1+x)-x\over x}} \\
=&\:e^{\lim\limits_{x\to0}-\frac1x\cdot\frac12x} \\
=&\:e^{-\frac12}
\end{align}
$$
...
**例 2** $\lim\limits_{x\to0}{\sin x-x\cos x\over x^3}$
