...
**例** 设 $f(x)$ 二阶可导，满足 $f(-1)=\frac13$，$f(0)=\frac16$，$f(2)=\frac{17}6$，证明：存在 $\xi\in(-1,2)$，使得 $f''(\xi)=1$。
...
**例 2** $a,b\in\bf R$，求证 $\left|\arctan a-\arctan b\right|\le|b-a|$。
...
**例 3** 证明当 $x>0$ 时，$\dfrac x{1+x}<\ln(1+x)<x$。
...
**例** 设 $y=f(x)$ 在 $x=0$ 的某邻域内具有 $n$ 阶导数，且 $f(0)=f'(0)=\cdots=f^{(n)}(0)$，试用柯西中值定理证明：$\dfrac{f(x)}{x^n}=\dfrac{f^{(n)}(\theta x)}{n!}$，$\theta\in(0,1)$。
