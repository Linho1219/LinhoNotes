...
**区间**一定是连续的，$ (-a,0)\cup (0,a) $ 就不是一区间。

### 邻域

对于实数 $\delta >0$ ，$(a-r,a+r)$ 称为 $a$ 的 $\delta $ **邻域**，记作 $U(a,\delta)$。
...
绝对值函数：$y = | x | = \left\{ \begin{array}{ll}   { - x,} & {x < 0,}  \\     { x,} & {x \ge 0.}  \\ \end{array} \right.$

取整函数：$y = \left[ x \right]$（不大于 $x$ 的最大整数）。

符号函数 $y = {\rm sgn} x = \left\{ \begin{array}{ll}    { - 1,} & {x < 0,} \\    {0,} & {x = 0,} \\    {1,} & {x > 0.} \end{array} \right.$
...
周期函数**不一定**有最小正周期——例：狄利克雷函数 $D(x) = \left\{ \begin{array}{ll}    {1,} & {x \in {\bf{Q}},} \\    {0,} & {x \in {{\bf{Q}}^C}.} \end{array} \right.$
...
双曲正弦 $\sh x = {{e^x} - {e^{ - x}} \over 2}$，奇函数单调增；

双曲余弦 $\ch x = {{e^x} + {e^{ - x}} \over 2}$，偶函数先减后增无零点，纵轴截距为1；

双曲正切 $\def\th{\operatorname{th}} \th x = {{\sh x} \over \ch x} = {{e^x} - {e^{ - x}} \over {e^x} + {e^{ - x}}}$，奇函数单调增；
...
\begin{align*}
  ...
  {\rm sh}2x &= 2{\rm sh}x{\rm ch}x &
  \sin 2x&=2 \sin x \cos x \\
  {\rm ch}2x&={\rm ch}^2x+{\rm sh}^2x & \cos 2x&=\cos^2 x - \sin^2 x \\
...
> [!warning]
> 在相当多的公式中，双曲函数的正负号和三角的不一样，而还有很多是一样的，一定要注意区分。其原因和 $i$ 有关系：
>
> 双曲函数的定义是基于指数函数：
>
...
>
>  在三角函数的定义中，由于涉及 $i$（虚数单位），在指数函数展开过程中，虚数的性质$i^2 = -1$直接影响了公式中的正负号

根据定义式反求可得：

反双曲正弦 $\def\arsh{\operatorname{arsh}} y = \arsh x = \ln \left( {x + \sqrt {{x^2} + 1} } \right)$；

反双曲余弦 $\def\arch{\operatorname{arch}} y = \arch x = \ln \left( {x + \sqrt {{x^2} - 1} } \right)$；

反双曲正切 $\def\arth{\operatorname{arth}} y = \arth x = {1 \over 2}\ln {{1 + x} \over {1 - x}}$；
