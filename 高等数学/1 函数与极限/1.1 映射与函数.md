...
绝对值函数：$y = | x | = \left\{ \begin{array}{l}
   { - x,} \hfill & {x < 0,}  \\ 
   { x,} \hfill & {x \ge 0.}  \\ \end{array} \right.$

取整函数：$y = \left[ x \right]$（不大于 $x$ 的最大整数）。

符号函数 $y = {\mathop{\rm sgn}} x = \left\{ \begin{array}{ll}
   { - 1,} & {x < 0,} \\ 
   {0,} & {x = 0,} \\ 
...
周期函数**不一定**有最小正周期——例：狄利克雷函数 $D(x) = \left\{ \begin{array}{ll}
   {1,} & {x \in {\bf{Q}},} \\ 
   {0,} & {x \in {{\bf{Q}}^C}.} \end{array} \right.$
