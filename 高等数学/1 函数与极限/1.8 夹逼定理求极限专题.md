...
> $$
>
> 在此基础上，用最小的那个数放缩，可以得到另一个结论：有限个正数的 $n$ 次方的和的 $n$ 次方根在 $n\to0$ 时的极限为其中最小的那个正数。
> 
> $$
> \lim_{n\to0}\sqrt[n]{\sum a_i^n}=a_\min
> $$

**变式** 设 $0<a<b$，口算 $\lim\limits_{n\to\infty}(a^{-n}+b^{-n})^\frac1n$ 的值。
