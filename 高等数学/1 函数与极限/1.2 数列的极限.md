...
**推论：1.如果数列 $\{x_n\}$ 从某项起有 $x_n\ge0$（或 $x_n \le 0$），且 $\lim\limits_{x\to\infty}x_n=a$，那么 $a\ge0$（或 $a\le0$）。**

**2.(保序性)如果 $\lim\limits_{x\to\infty}x_n=a$，$\lim\limits_{y\to\infty}y_n=b$且 $a>b$（或 $a<b$），那么存在正整数 $N$，当 $n>N$ 时，都有 $x_n>y_n$（或 $x_n<y_n$）。**取 $\varepsilon \in (0,\abs{a-b})$，很容易证明
