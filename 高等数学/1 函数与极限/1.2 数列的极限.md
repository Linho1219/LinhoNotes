...
- $ \lim\limits_{x \to\infty} {{\rm{q}}^n} = 0(\left| q \right| < 1) $
- 有界数列 $ \{x_n\} $ 与无穷小量 $ \lim\limits_{n\to\infty}y_n=0 $ 之积还是无穷小量
- $ \lim\limits_{n\to\infty}x_n=a \Leftrightarrow \forall\{a_{n_k}\},\lim\limits_{n_k\to\infty}x_{n_k}=a $ 逆否命题：$ \lim\limits_{n\to\infty}x_n $ 不存在 $ \Leftrightarrow $  有一个子列不收敛or两个子列收敛但极限不同
- $ \lim\limits_{n\to\infty}x_n=a \Leftrightarrow \lim\limits_{n\to\infty}x_{2n}=a 且 \lim\limits_{n\to\infty}x_{2n+1}=a$ 