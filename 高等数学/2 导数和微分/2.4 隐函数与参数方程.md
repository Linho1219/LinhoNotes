...
**例 1** 求 $e^y+xy-e=0$ 确定的 $\dfrac{{\rm d}y}{{\rm d}x}$。
...
**例 2** 求 $y^5+2y-x-3x^7=0$ 确定的 $\left.\dfrac{{\rm d}y}{{\rm d}x}\right|_{x=0}$。
...
**例 3** 求 $x-y+\frac12\sin y=0$ 确定的二阶导数 $\dfrac{{\rm d}^2y}{{\rm d}x^2}$。
...
**例 2** 求 $y=\sqrt{\dfrac{(x-1)(x-2)}{(x-3)(x-4)}}$ 的导数。
