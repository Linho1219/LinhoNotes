...
上面的代码中，我们通过判断是否定义_WIN32或_WIN64来判断编译器是否运行在Windows系统下，通过判断是否定义__linux来判断编译器是否运行在Linux系统下。
...
\#define还可以用于定义宏。在这种情况下，编译器会直接将对应的宏替换成这部分内容。例如：

```cpp
#define PI 3.1415926
```

这样当程序中出现PI时，编译器会直接将其替换成3.1415926。这种特性在一些我们不希望功能额外占用内存的场合，或者要为一个固定值赋予语义时十分有用。例如：

```cpp
#define GPIOB_BASE            (APB2PERIPH_BASE + 0x00000C00UL) // 这里是STM32F103C8T6中GPIOB寄存器的起始地址，使用APB2PERIPH地址作为基地址是因为GPIOB挂在APB2时钟下
#define GPIOB               ((GPIO_TypeDef *)GPIOB_BASE)
#define GPIO_PIN_3                 ((uint16_t)0x0008)  /* Pin 3 selected    */
#define BUTTON_Pin            GPIO_PIN_3  
#define BUTTON_GPIO_Port      GPIOB
/* 按键状态（低电平有效）：true=按下，false=未按 */
#define Button_GetState() (HAL_GPIO_ReadPin(BUTTON_GPIO_Port, BUTTON_Pin) == GPIO_PIN_RESET) // 读取B3口的电平，低电平返回true
```
...
上面的代码声明了一个针对存储string的vector对象的迭代器（将在[后续](第十二章-C++中的STL与标准库算法#1. STL容器)讲解STL容器与迭代器模式）。如果我们每次要声明迭代器时都需要写这么一长串比命还长的类型，那多这么来几次我们就会疯的。我们可以用auto来简化上面的迭代器声明：
...
- private：表示标记为该等级的成员和方法仅能被此对象访问（友元类除外，将在[后续](第七章-C++类的运算符重载和友元#2. 友元)说明）；
- public：表示标记为该等级的成员和方法能被以任意方式访问；
- protected：表示标记为该等级的成员和方法仅能被此对象及此对象派生的对象访问（将在[后续](第八章-C++中的继承与多态#1. 继承)讲解类的继承机制）
...
这样我们就能够用greater函数比较两个对象，将较大的那个Number对象返回了。这项工作能够使用运算符重载更加优雅的完成（将在[后续](第七章-C++类的运算符重载和友元#1. 运算符重载)介绍运算符重载）。
