...
在[第一章](第1章-C++基础入门)中，我们简单讲解了函数的基本概念——函数的参数、返回值、main函数、函数体等。接下来我们继续来详细讲解函数的特性。
## 1. 函数的形参与实参
...
```
其中template关键字向C++编译器描述了下面的函数或类（在[后面](第6章-C++中的多文件编译、变量高级与类基础)具体讲解类）是一个模板。class T指定了模板变量的数据类型名。T的具体类型由函数的实参决定。如果我们向上面的swap()中传入了两个int，那么函数会自动适配为针对int处理。
