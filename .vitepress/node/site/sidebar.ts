...
export const sidebarConfig: SidebarConfig = {
  ignProjects: ['.github', '.git', '.vitepress', 'node_modules', 'patches', 'public'],
  ignDirs: ['images'],
  ignFiles: ['index.md'],
  maxDepth: 2,
}
...
function compareFileName(a: string, b: string) {
  ...
  return indexA.big - indexB.big || indexA.small - indexB.small || pinyin.compare(a, b)
}
...
const folderFilter = (config: DirConfig) => (dirent: fs.Dirent) =>
  dirent.isDirectory() && !ignDirs.includes(dirent.name) && !config.ignore?.some(test(dirent.name))
...
function generateSidebar(folderPath: string, depth = 0): DefaultTheme.SidebarItem[] {
