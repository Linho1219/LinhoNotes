...
export default function ins_plugin(md: MarkdownIt, options: customWrapperOptions) {
  const { marker: MARKER, tag: TAG, typeName: TYPE = TAG.toLowerCase() } = options
  ...
  function postProcess(state: StateInline, delimiters: StateInline.Delimiter[]) {
    ...
    while (loneMarkers.length) {
      ...
      while (j < state.tokens.length && state.tokens[j].type === 'cloze_close') {
        j++
      }
