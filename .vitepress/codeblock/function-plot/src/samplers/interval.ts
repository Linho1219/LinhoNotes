...
function interval1d({
  ...
  xScale,
  yScale,
}: SamplerParams): IntervalSamplerResult {
...
const syncSamplerInterval: SamplerFn = function sampler(
  ...
  switch (samplerParams.d.fnType) {
    ...
    default:
      throw new Error(
        samplerParams.d.fnType +
          " is not supported in the `interval` sync sampler"
      );
  }
};
...
const asyncSamplerInterval: AsyncSamplerFn = async function sampler(
  ...
  switch (samplerParams.d.fnType) {
    ...
    default:
      throw new Error(
        samplerParams.d.fnType +
          " is not supported in the `interval` async sampler"
      );
  }
};
