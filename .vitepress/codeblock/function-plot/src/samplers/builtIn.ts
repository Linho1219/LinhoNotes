...
function split(
  ...
  while (i < data.length) {
    ...
    if (samplerGroup.length > 1) {
      deltaX =
        samplerGroup[samplerGroup.length - 1][0] -
        samplerGroup[samplerGroup.length - 2][0];
      oldSign = newSign;
    }
    ++i;
  }
...
const sampler: SamplerFn = function sampler(
  ...
  switch (samplerParams.d.fnType) {
    ...
    default:
      throw Error(
        samplerParams.d.fnType + " is not supported in the `builtIn` sampler"
      );
  }
};
