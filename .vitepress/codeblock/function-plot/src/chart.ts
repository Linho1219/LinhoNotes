...
export class Chart extends EventEmitter.EventEmitter {
  ...
  internalVars() {
    ...
    // inner width/height
    this.meta.width =
      (this.options.width || globals.DEFAULT_WIDTH) -
      margin.left -
      margin.right;
    this.meta.height =
      (this.options.height || globals.DEFAULT_HEIGHT) -
      margin.top -
      margin.bottom;
  ...
  initializeAxes() {
    ...
    const xDomain = (this.meta.xDomain = (function (
    ...
    const yDomain = (this.meta.yDomain = (function (
      axis: FunctionPlotOptionsAxis
    ) {
      if (axis.domain) {
        return axis.domain;
    ...
    this.meta.xScale
      ...
      .range(
        this.options.x.invert ? [this.meta.width, 0] : [0, this.meta.width]
      );
    ...
    this.meta.yScale
      ...
      .range(
        this.options.y.invert ? [0, this.meta.height] : [this.meta.height, 0]
      );
  ...
  drawGraphWrapper() {
    ...
    root
      ...
      .attr(
        "width",
        this.meta.width + this.meta.margin.left + this.meta.margin.right
      )
      .attr(
        "height",
        this.meta.height + this.meta.margin.top + this.meta.margin.bottom
      );
  ...
  updateAxes() {
    ...
    if (this.options.x.position === "sticky") {
      ...
      yTranslation =
        yTranslation > this.meta.height ? this.meta.height : yTranslation;
      ...
      canvas
        ...
        .attr(
          "transform",
          "translate(0," +
            (this.meta.height / 2 - yTranslation + this.meta.height / 2) +
            ")"
        );
    }
    ...
    if (this.options.y.position === "sticky") {
      ...
      xTranslation =
        xTranslation > this.meta.width ? this.meta.width : xTranslation;
      canvas
        .select(".y.axis")
  ...
  setUpEventListeners() {
    ...
    const eventsThisInstance: EventsWithListener = {
      ...
      zoom: function zoom({ transform }: any) {
        ...
        const xScaleClone = transform
          .rescaleX(self.meta.zoomBehavior.xScale)
          .interpolate(d3InterpolateRound);
        const yScaleClone = transform
          .rescaleY(self.meta.zoomBehavior.yScale)
          .interpolate(d3InterpolateRound);
