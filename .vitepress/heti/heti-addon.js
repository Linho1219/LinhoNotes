...
/**
 * Originally authored by Sivan
 * Repo: https://github.com/sivan/heti , licensed under MIT
 * Modified by Linho for LinhoNotes
 * 此版本为修改版。主要修改内容为
 *  1. 将 pangu 空格改为使用普通空格实现
 *  2. 将公式前后添加的不可见字符视为数字实现 pangu
 * 很 Hack，作为过渡使用，等待后续原生 CSS 实现
 */
...
export class Heti {
  ...
  spacingElement($$elm) {
    ...
    findAndReplaceDOMText(
      $$elm,
      Object.assign({}, commonConfig, {
        find: new RegExp(` [${REG_BD_OPEN}${REG_BD_CLOSE}]`, "g"),
        replace: (portion) => portion.text.trim(),
      })
    );
    findAndReplaceDOMText(
      $$elm,
    ...
    // Mod: 擦屁股，把公式添加的不可见字符拿掉，防止复制粘贴和字数统计出问题
    findAndReplaceDOMText(
      $$elm,
      Object.assign({}, commonConfig, {
        find: /\u200e/g,
