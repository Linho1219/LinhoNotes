...
const handleTouch = ({ touches }: TouchEvent) => {
  ...
  const onTouchMove = ({ touches }: TouchEvent) => {
    ...
    if (touches.length === 1) {
      ...
      const newScale = limitRange((lastScale * getDistance(dx, dy)) / begin.distance)
      const newPosition = {
  ...
  const onTouchEnd = ({ touches }: TouchEvent) => {
    ...
    if (
      ...
      getDistance(originalPosition.x - position.x, originalPosition.y - position.y) <
        DOUBLETAP_DISTANCE
    ) {
      // 判定为点击而非拖动
...
.viewer-img {
  ...
  background-image:
    linear-gradient(45deg, var(--gray) 25%, transparent 0, transparent 75%, var(--gray) 0),
    linear-gradient(45deg, var(--gray) 25%, transparent 0, transparent 75%, var(--gray) 0);
  background-position:
    0 0,
    var(--size) var(--size);
