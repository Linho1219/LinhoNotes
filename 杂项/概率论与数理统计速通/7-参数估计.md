...
可以证明：样本均值、样本方差（分母 $n-1$ 那个）是无偏估计量，**但矩估计和最大似然估计给出的方差（分母 $n$）不是无偏估计量**。
...
> | 估计 $\sigma$             | $\chi^2=\dfrac{(n-1)S^2}{\sigma^2}\sim \chi^2(n-1)$ |
>
...
$$
\begin{gathered}
\chi^2=\frac{(n-1)S^2}{\sigma^2}=\frac{8\times0.0325}{\sigma^2}\sim\chi^2(8) \\
